...
const getAll = formdata => (predicate) => {
  let i = 0;
  const result = [];
...
var css = {"root":"button_root__3QOnQ","_$root":"button_root__3QOnQ"};
...
var css$1 = {"root":"progress_root__2xqYW","bar":"progress_bar__2vGxy","_$root":"progress_root__2xqYW","_$bar":"progress_bar__2vGxy"};
...
const radio = ({
  ...
  <label class$=${cn(ui$2.root, ui$2['size-medium'], css$2.root)} for$=${label}>
    <input
      checked$=${checked}
      class$=${cn(ui$2.input, css$2.input, classname)}
      id$=${label}
      name$=${name}
      type="radio"
      value=${value}
    />
    ${(label || children) && html$1`<span class$=${ui$2.label}>${label || children}</span>`}
  </label>
`;
...
const result = (data) => {
  ...
  return html$1`
    <div class$=${classString({ [cn(css$3.text)]: true, [css$3.active]: data.winner })}>${data.label}</div>
    <div class$=${cn(css$3.aftertext)}>${value}</div>
  `
};
...
const combineMixins = it => compose(...apply(it));

const withPoll = (base, initialConfig = {}) => class extends base {
  constructor (props) {
    super(props);
...
class PollElement extends LitElement {
  ...
  __processFormData (fd) {
    const getAll$$1 = getAll(fd);

    return this.list.reduce(
      (acc, it) => ~acc[1].indexOf(it.name)
        ? acc
        : fd.get(it.name)
          ? [acc[0].concat(getAll$$1(it.name)), acc[1].concat(it.name)]
          : acc,
      [[], []]
    )
  }
