...
const radio = ({
  ...
  <label class$=${cn(ui.root, ui['size-medium'], css$1.root)} for$=${label}>
    <input
      checked$=${checked}
      class$=${cn(ui.input, css$1.input, classname)}
      id$=${label}
      name$=${name}
      type="radio"
      value=${value}
...
const button = ({
  ...
  <button
    disabled=${disabled}
    class$=${!disabled ? buttonCls : cn(buttonCls, ui$1.disabled, css$1$1.disabled)}
    form=${forEl}
    on-click=${onclick}
  >${text}</button>
`;
...
const Poll$1 = _withStyle()(PollElement, css);

export { Poll$1 as Poll };
